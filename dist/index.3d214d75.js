...
wordList = wordList.concat(wordList2);
// let wordList = [
//   'apple',
//   'bagel',
//   'candy',
...
function handleKey(key) {
    ...
    updateGrid();
    saveGame();
    if (history.length === 6 && history[history.length - 1] !== answer) setTimeout(()=>alert(answer)
    , 100);
}
...
function buildGrid() {
    ...
    for(let i = 0; i < ROWS; i++){
        ...
        for(let j = 0; j < COLS; j++){
            ...
            if (i < numAttempts) surface.style.transitionDelay = j * 100 + 'ms';
            else surface.style.transitionDelay = j * 300 + 'ms';
...
function drawAttempt(row, attempt, isSolved) {
    let letters = new Map();
    for (const [key, count] of countOfLetter)letters.set(key, count);
    // set greens
    ...
    for(let i1 = 0; i1 < COLS; i1++){
        let cell = row.children[i1];
        let surface = cell.firstChild;
        let back = surface.children[1];
        let bgColor = getBgColor(attempt, i1);
        let count = letters.get(attempt[i1]);
        if (bgColor === GREEN) continue;
        if (bgColor === BLUE && count > 0) {
            letters.set(attempt[i1], --count);
            back.style.backgroundColor = bgColor;
            back.style.borderColor = bgColor;
        } else {
    ...
    for(let i2 = 0; i2 < COLS; i2++){
        let cell = row.children[i2];
        let surface = cell.firstChild;
        let front = surface.children[0];
        let back = surface.children[1];
        if (attempt[i2] !== undefined) {
            front.textContent = attempt[i2];
            back.textContent = attempt[i2];
        } else {
        ...
        if (attempt[i2] !== undefined) front.style.borderColor = LIGHTGREY;
...
function getBgColor(attempt, i) {
    ...
    if (attemptLetter === correctLetter) return GREEN;
    if (answer.indexOf(attemptLetter) !== -1) return BLUE;
    return MIDDLEGREY;
}
function buildRow(letters, isLast) {
    ...
    if (isLast) {
        ...
        key.className = 'keyboardBtn';
        key.classList.add('specialKey');
        key.textContent = 'Enter';
        key.onclick = handleKey('enter');
        row.appendChild(key);
    }
    for (const letter2 of letters){
        ...
        row.appendChild(key);
        keyboardButtons.set(letter2, key);
    }
    if (isLast) {
        let key = document.createElement('button');
        key.className = 'keyboardBtn';
        ...
        key.onclick = ()=>handleKey('backspace')
        ;
        row.appendChild(key);
    }
...
function buildKeyboard() {
    buildRow('qwertyuiop', false);
    buildRow('asdfghjkl', false);
...
function updateKeyboard() {
    ...
    for (const attempt of history)for(let i = 0; i < attempt.length; i++){
        let key = attempt[i];
...
function loadGame() {
    ...
    if (data !== null && data.answer === answer) history = data.history;
}
function saveGame() {
...
for (const letter of answer){
    ...
    if (count === undefined) countOfLetter.set(letter, 1);
    else countOfLetter.set(letter, ++count);
}
loadGame();
buildGrid();
updateGrid();
...
},{}]},["kn9T2","aPJuQ","bB7Pu"], "bB7Pu", "parcelRequire5444")
